  ...
  "An instance of LocalFileStore" must {
    ...
    "have an existsInStore method" which {
      ...
      "completes with false if a file doesn't exist at the specified path in the file store" in withLocalFileStore {
        t =>
          val testHash: SHA256Hash =
            SHA256Hash("482bfece08d11246f41ce3dc43480e1b61659fbe0083b754bee09b44b940ae6c")
    ...
    "have an add method" which {
      ...
      "completes with Added after successfully copying a file to the file store" in withLocalFileStore {
        t =>
          t.add(add).unsafePerformSync should equal(Added)
      }
      "completes with Exists when trying to copy a file to a path where one already exists" in withLocalFileStore {
        t =>
          t.add(add)
            .flatMap { _ =>
              t.add(add)
    ...
    "have a remove method" which {
      ...
      "completes with Removed after successfully deleting a file from the file store" in withLocalFileStore {
        t =>
          t.add(add)
            .flatMap { _ =>
              t.remove(testPDFHash)
      ...
      "completes with NotFound when attempting to remove a file which doesn't exist in the file store" in withLocalFileStore {
        t =>
